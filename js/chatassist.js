...
function addChatMessage(platform, nickname, message, sticky, ext_args) {
    ...
    if(rawprint) {
        ...
        if(platform == "naver") {
            ...
            if(ext_args.isStreamer) {
                message = message.replace(/~([^ ]+)+(?: )*(.+)*/gi, replaceCommand);
                nickname = '<span style="vertical-align: middle; display: inline-block; width: 16px; height: 16px; border-radius: 50%; background: #e60000; color: #fff; font-size: 10px; text-align: center; line-height: 16px;" class="badge streamer" title="채널 소유자">&#9654;</span>&nbsp;' + nickname;
            }
            ...
            if(ext_args.isMod) {
                nickname = "<b>" + nickname + "</b>";
                message = message.replace(/~([^ ]+)+(?: )*(.+)*/gi, replaceCommand);
                nickname = '<span style="vertical-align: middle; display: inline-block; width: 16px; height: 16px; color: #5e84f1; font-size: 14px; line-height: 16px;" class="badge mod" title="모더레이터">&#9881;</span>&nbsp;' + nickname;
            }
        } else {
            // 스트리머 뱃지
            if(ext_args.isStreamer) {
...
function connect_yt() {
    ...
    var ytApiKey = window.config.ytApiKey;

    if (!ytApiKey) {
        addChatMessage("error", "YouTube 연결 오류", "YouTube API 키가 필요합니다. ytApiKey 파라미터를 설정해주세요.", true, false);
        return;
    }
...
function yt_pollChat(liveChatId, apiKey, pageToken) {
    var url = 'https://www.googleapis.com/youtube/v3/liveChat/messages?liveChatId=' + liveChatId + '&part=snippet,authorDetails&key=' + apiKey;
    if (pageToken) {
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4) {
            if (xhr.status === 200) {
                ...
                var pollingInterval = data.pollingIntervalMillis || 5000;

                // 첫 폴링(pageToken 없음)은 기존 채팅 내역이므로 건너뛰기
                if (pageToken) {
                    for (var i = 0; i < data.items.length; i++) {
                        var item = data.items[i];
                        var snippet = item.snippet;
                        var authorDetails = item.authorDetails;

                        if (snippet.type !== "textMessageEvent" && snippet.type !== "superChatEvent") continue;

                        var nickname = authorDetails.displayName;
                        var message = snippet.type === "superChatEvent"
                        ...
                        var ext_args = {};
                        ext_args.isStreamer = authorDetails.isChatOwner;
                        ext_args.isMod = authorDetails.isChatModerator;
                        ext_args.rawprint = false;
                        ext_args.emotes = void 0;
                        ext_args.color = void 0;
                        ext_args.subscriber = authorDetails.isChatSponsor;
                        ext_args.id = authorDetails.channelId;

                        addChatMessage("youtube", nickname.htmlEntities(), message.htmlEntities(), false, ext_args);
                    }
                }
                ...
                console.error("YouTube API 할당량 초과 또는 권한 오류");
