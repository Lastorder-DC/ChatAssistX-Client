...
/**
 * WebSocket 서버를 생성한다.
 * @param {Object} options - 서버 설정
 * @param {number} [options.port] - 포트 번호 (기본값: process.env.PORT || 8090)
 * @param {boolean} [options.devMode] - 개발 모드 (origin 체크 비활성화)
 * @param {number} [options.pingTimeout] - ping 타임아웃 (ms)
 * @param {number} [options.pingCheckInterval] - ping 체크 주기 (ms)
 * @param {number} [options.sessionCleanupDelay] - 세션 정리 대기 시간 (ms)
 * @param {Function} [options.onStartSession] - 채널 세션 시작 시 호출되는 콜백 (테스트용)
 */
function createServer(options = {}) {
    const port = options.port || process.env.PORT || 8090;
    const devMode = options.devMode !== undefined ? options.devMode : process.argv.includes('--dev');
    // 60초 이상 ping이 없는 클라이언트 연결 해제
    const pingTimeout = options.pingTimeout || 60000;
    // 클라이언트 ping 주기(30초)보다 짧게 체크하여 타이밍 이슈 방지
    const pingCheckInterval = options.pingCheckInterval || 10000;
    // 세션 정리 대기 시간 (새로고침 시 재연결 허용)
    const sessionCleanupDelay = options.sessionCleanupDelay || 10000;

    // youtubei.js 모듈 지연 로딩 (테스트에서 불필요한 의존성 로딩 방지)
    let _ytModule = null;
    function getYtModule() {
        if (!_ytModule) _ytModule = require('youtubei.js');
        return _ytModule;
    }
    ...
    console.log(`YouTube Live Chat relay server ${PROGRAM_VERSION} started on port ${port}${devMode ? ' (dev mode - origin check disabled)' : ''}`);
    ...
    function unsubscribeClient(ws, channel) {
        ...
        if (session.clients.size === 0) {
            console.log(`No more clients for ${channel}, waiting ${sessionCleanupDelay / 1000}s before stopping session`);
    ...
    async function subscribeClient(ws, channel) {
        ...
        try {
            if (options.onStartSession) {
                await options.onStartSession(channel, session, broadcast);
            } else {
    ...
    async function startChannelSession(channel, session) {
        const { Innertube, YTNodes } = getYtModule();
    ...
    function gracefulShutdown() {
        ...
        // 세션 유무와 관계없이 연결된 모든 클라이언트에게 종료 알림 메시지 전송
    ...
    function close() {
        ...
        for (const [, session] of channelSessions) {
            ...
            if (session.livechat) {
                try { session.livechat.stop(); } catch (e) {}
            }
        }
        channelSessions.clear();
        return new Promise((resolve) => {
            for (const client of wss.clients) {
                try { client.close(1001, 'Server closing'); } catch (e) {}
            }
            wss.close(resolve);
        });
    }

    return { wss, channelSessions, gracefulShutdown, close };
}
